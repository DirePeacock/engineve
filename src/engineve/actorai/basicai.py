...
class BasicAI():
    ...
    def get_available_game_move_keys(cls, actor_id, state):
        move_keys = [key for key, game_move in state.actors[actor_id].game_moves.items() if game_move.is_available(state)]
    ...
    def get_nearest_enemy_id(actor_id, state):
        enemy_dists = {}
        for enemy_id in [enemy_id for enemy_id in aiutils.get_enemy_ids(actor_id, state) if not state.actors[enemy_id].is_dead()]:
            enemy_dists[enemy_id] = pathingutils.measure_distance(state.actors[actor_id].loc, state.actors[enemy_id].loc)
