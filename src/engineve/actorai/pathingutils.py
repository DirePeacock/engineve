...
def measure_linear_distance(loc_a, loc_b):
    """get hypotenues length so that some things can look moresensible than dnd"""
    return math.sqrt((loc_a[0] - loc_b[0]) ** 2 + (loc_a[1] - loc_b[1]) ** 2)


# ASSUMPTION coords are all positive numbers for now
