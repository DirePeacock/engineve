...
class UseMovement(GameMove):
    ...
    def get_weight(self, state):
        target_in_already_range = any(weight for weight in self.weigh_targets(state).values() if weight <= 1)
    ...
    def get_targets(self, state):
        return [enemy_id for enemy_id in get_enemy_ids(self.actor_id, state) if 0 < state.actors[enemy_id].hp]
    ...
    def make_command(self, state, *args, **kwargs) -> ChangeLocCommand:
        ...
        target_distances = self.weigh_targets(state)

        # if any(tgt for tgt in self.weigh_targets(state).values() <= 1):
