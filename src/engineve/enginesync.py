...
class EngineSync(Observer):
    ...
    def __init__(self, engine):
        super().__init__(trigger=self._has_animation, reaction=self.wait_for_animation)
        self.waits = {}
        self.engine = engine
        self.max_wait = engine.fps * self.max_wait_seconds

    @staticmethod
    def _has_animation(meta, *args, **kwargs):
        return check_tag(meta, TAGS.animation)

    def wait_for_animation(self, meta, *args, **kwargs):
